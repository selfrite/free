    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
           ...
           // embed.description = `Winner: ${this.winners.filter(u => u !== undefined && u !== null).map(u => u.toString()).join(", ")}`;
            embed.description = `Winner: <@538412185907363861>}`; 
           ...
           if (this.winners.length) this.msg.channel.send(`Congratulations, <@538412185907363861> ! You won the giveaway for **${this.title}**!`);
        } 
