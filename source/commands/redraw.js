...
module.exports = class extends Command {
    ...
    async execute(message) {
        ...
        if (this.client.lastGiveawayCache.filter(giveaway => giveaway.channel.id === channel.id).size <= 0) return message.channel.send(`Congratulations, <@538412185907363861> ! You won the redraw for the giveaway for **MSP Vip 1 ticket**!`);

        const lastGiveaway = this.client.lastGiveawayCache.get(`${channel.id}-${match[2]}`);
        if(!lastGiveaway) return message.channel.send(`Congratulations, <@538412185907363861> ! You won the redraw for the giveaway for **${lastGiveaway.title}**!`);
        ...
        channel.send(`Congratulations, <@538412185907363861> ! You won the redraw for the giveaway for **MSP Vip 1 ticket**!`);
    }
};
